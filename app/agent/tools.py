...
def get_revenue_by_channel(
    ...
    """
    try:
        results = bq_service.get_revenue_by_channel(days)
        if not results:
        ...
        return f"Revenue by channel (last {days} days):\n" + "\n".join(lines)
    except Exception as e:
...
def get_channel_comparison(
    ...
    """
    try:
        results = bq_service.get_channel_comparison(days)
        if not results:
        ...
        return f"Channel comparison (last {days} days):\n" + "\n".join(lines)
    except Exception as e:
